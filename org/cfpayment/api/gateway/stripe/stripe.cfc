<!---
	...
	MerchantAccount = SecretKey
--->
<cfcomponent displayname="Stripe Gateway" extends="base" hint="Stripe Gateway" output="false">
	...
	<cffunction name="purchase" output="false" access="public" returntype="any" hint="Authorize + Capture in one step - only approach supported by Stripe">
		...
		<cfif structKeyExists(arguments.options, "description")>
			<cfset post["description"] = arguments.options.description />
		<cfelse>
			<cfset post["description"] = arguments.refid />
		</cfif>
	...
	<cffunction name="credit" access="public" output="false" returntype="any" hint="Returns an amount back to the previously charged account.  Default is to refund the full amount.">
	...
	<cffunction name="process" output="false" access="private" returntype="any">
		...
		<!--- add authentication --->		
		<cfset headers["authorization"] = "Bearer #getMerchantAccount()#" />
