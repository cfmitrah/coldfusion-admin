...
*/
	...
	if(structKeyExists(url, "path_info")){
		variables.cgiPathInfo = url.path_info;
	}
	request._fw1 = {
    ...
    public any function onRequestEnd() {
    	var renderDebug = ( structKeyExists( request, "hideDebug" ) && request.hideDebug ? false : variables.framework.trace);
    	if( renderDebug ){
        	frameworkTraceRender();
        }
    }
